"""Hàm vẽ dùng chung — CHỦ SỞ HỮU: Lộc (branch `eval-eng`).

Task T6.0. Phong và Thông **gọi** các hàm này để mọi figure trong report cùng một style;
không ai tự vẽ kiểu riêng.

Quy ước lưu file: ``reports/figures/{owner}_{task}_{name}.png``
ví dụ ``phong_t3_shot_heatmap.png``, ``thong_t5_elbow_scaled.png``.
"""
...
def plot_elbow(curve: pd.DataFrame, title: str, out_path) -> None:
    ...
    raise NotImplementedError("T6.0 @loc")
