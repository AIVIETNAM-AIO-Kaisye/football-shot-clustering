...
**File được commit trực tiếp:** `features.py` · `preprocess.py` · `clustering.py` · `selection.py` · `scripts/04,05` · `tests/test_features.py` · `notebooks/B_modeling.ipynb` · `figures/B_*` · file này

---

## NGÀY 1 — Geometry features *(GATE 1)*

- [ ] **T1.3a** `@B` `#data` `#p0` `#todo` — `distance_to_goal` `est 1h`
      ↳ `hypot(120 − x, 40 − y)`, đơn vị **yard** (không phải mét)

- [ ] **T1.3b** `@B` `#data` `#p0` `#todo` — `angle_to_goal` bằng định lý cosin `est 2.5h`
...
- [ ] **T4.1** `@B` `#model` `#p0` `#todo` — Tách 3 nhóm cột `X` / `Y_hidden` / `ID` `est 1h`
      ↳ ⚠️ `Y_hidden` gồm cả `end_x/end_y/end_z` — dễ quên, xem ADR-006
      ↳ out: `data/processed/y_hidden.csv`

- [ ] **T4.2** `@B` `#model` `#p0` `#todo` — One-hot `body_part`, `technique`, `shot_type` `est 1h`

- [ ] **T4.3** `@B` `#model` `#p1` `#todo` — Xử lý missing `est 1h`
      ↳ ghi rõ **số dòng bị ảnh hưởng** vào `DECISIONS.md`
      ↳ note: `gk_*` thiếu khi freeze_frame không có GK — quyết định impute hay bỏ cột

- [ ] **T4.4** `@B` `#model` `#p0` `#todo` — `StandardScaler` → 2 phiên bản X `est 1.5h`
...
- [ ] **T4.5** `@B` `#model` `#p1` `#todo` — Lưu file kèm `shot_id` để join ngược `Y_hidden` `est 0.5h`
      ↳ check: 2 file cùng số dòng, cùng thứ tự

- [ ] **T5.0a** `@B` `#model` `#p0` `#todo` — `clustering.py`: runner K-Means quét k `est 1.5h`
      ↳ `run_kmeans_sweep(X, k_range) -> {k: (labels, inertia, model)}`
      ↳ ⚠️ API này **A cũng dùng** ở D3 → đóng băng chữ ký hàm, đổi phải báo A

- [ ] **T5.0b** `@B` `#model` `#p0` `#todo` — `selection.py`: Elbow + Silhouette `est 1.5h`
...
- [ ] **T5.2** `@B` `#model` `#p0` `#todo` — K-Means trên `X_scaled`, k=2..10, cả 3 phương pháp `est 2.5h`
      ↳ out: `figures/B_t5_elbow_scaled.png`, `B_t5_silhouette_scaled.png`, `B_t5_gap_scaled.png`
      ↳ out: `reports/tables/t5_k_selection_scaled.csv`

- [ ] **T5.3** `@B` `#model` `#p0` `#todo` — Tính **ARI** giữa nhãn scaled và unscaled `est 1h`
      ↳ blocked-by: T5.1d (@A, nộp 14:00)
      ↳ ⚠️ so sánh ở **cùng một k** thì mới có nghĩa
      ↳ diễn giải: ARI ≈ 0 ⇒ scaling đổi hoàn toàn kết quả · ARI ≈ 1 ⇒ hầu như không đổi

- [ ] **T5.4** `@B` `#model` `#p0` `#todo` — Chốt phiên bản cụm chính thức `est 1h`
...
- [ ] **T5.4b** `@B` `#model` `#p1` `#todo` — Mô tả centroid từng cụm `est 1.5h`
...
- [ ] **T7.4** `@B` `#docs` `#p0` `#todo` — Viết phần *Thiết kế thí nghiệm* + *Preprocessing* cho report `est 2h`
      ↳ nhấn: L2 là **biến kiểm soát**, không phải biến thí nghiệm (ADR-009)

- [ ] **T7.5** `@B` `#infra` `#p1` `#todo` — Review PR của A và C `est 1.5h`

- [ ] **T7.6** `@B` `#docs` `#p1` `#todo` — Rà lại figures nhánh model, thống nhất bảng màu theo cụm `est 1h`
