...
\end{equation}
The expected value $E_n^*\{\cdot\}$ is calculated by generating $B = 10$ reference datasets from a uniform distribution over the bounding box of the empirical feature ranges (Tibshirani et al.'s standard uniform reference method).
...
\begin{minipage}{0.8\linewidth}
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}X >{\centering\arraybackslash}X@{}}
    ...
    \midrule
    StandardScaler vs. RobustScaler & \textbf{0.930} \\
    Unscaled vs. StandardScaler & 0.296 \\
    Unscaled vs. RobustScaler & 0.292 \\
    \bottomrule
  \end{tabularx}
\end{minipage}
\end{table}

The high agreement between StandardScaler and RobustScaler ($\text{ARI} = 0.930$) indicates that both scaling methods produce consistent cluster structures. In contrast, the poor agreement with Unscaled clustering ($\text{ARI} \approx 0.29$) confirms that unscaled clustering is heavily distorted by touch volume. RobustScaler was chosen for all subsequent modeling because of its resilience to skewed distributions and outliers.
...
\begin{itemize}
  \item \textbf{Elbow Method (WCSS):} The WCSS curve exhibits a distinct bend with diminishing marginal variance reduction flattening out around $k=5$.
  \item \textbf{Silhouette Score:} The Silhouette score reaches its numerical maximum at $k=2$ ($\approx 0.454$ for RobustScaler). However, qualitative inspection shows that $k=2$ merely separates defensive and attacking players—a partition too coarse for actionable tactical profiling. Between $k=4$ and $k=7$, the score stabilizes in a robust plateau ($S(5) \approx 0.203$), where tactical roles are cleanly segmented without over-fragmentation.
  \item \textbf{Gap Statistic:} The Gap Statistic increases monotonically across $k \in [2, 10]$ without converging under Tibshirani's 1-SE rule. In a 29-dimensional continuous feature space, monotonic dispersion growth is a well-documented geometric effect; hence, the empirical choice of $k=5$ is firmly anchored on the Elbow heuristic and validated by high cross-scaler stability ($\text{ARI} = 0.930$).
\end{itemize}
...
\]
\[
\text{NMI (4 Position Groups)} \approx 0.546, \quad \text{NMI (22 Detailed Positions)} \approx 0.508
\]

Examining the feature distributions of each cluster reveals five clear tactical archetypes:
\begin{itemize}
  \item \textbf{Cluster 0 (Defensive Anchors \& Center-Backs -- 194 players):} High purity with the broad Defense category ($97.4\%$, with central defenders forming $91.8\%$ of the cluster / 178 players). Defined by high clearance rates (\texttt{clearances\_p90} mean $5.54$, median $5.46$), aerial dominance, and deep pitch positioning (\texttt{avg\_location\_x} mean $42.71$, median $41.69$).
  \item \textbf{Cluster 1 (Clinical Finishers \& Primary Forwards -- 152 players):} Dominated by the Attack category ($95.4\%$, with central strikers forming $78.3\%$ / 119 players alongside direct goal-scoring wide attackers). Characterized by high shot volume (\texttt{shots\_p90} mean $2.50$, median $2.41$), non-penalty goal rate (\texttt{np\_goals\_p90} mean $0.34$), and frequent penalty-box presence (\texttt{touches\_in\_box\_p90} mean $14.36$).
  \item \textbf{Cluster 2 (Goalkeepers -- 73 players):} Achieves $100\%$ pure separation ($73/73$). Defined by near-zero outfield actions and minimal progression upfield (\texttt{avg\_location\_x} mean $9.62$).
  \item \textbf{Cluster 3 (Creative Playmakers \& Dynamic Wingers -- 198 players):} Combines wingers and wide midfielders ($60.1\%$, 119 players) with central attacking/creative midfielders ($38.9\%$, 77 players including 30 CAMs). Characterized by high key passes (\texttt{key\_passes\_p90} mean $1.56$, median $1.50$), crosses (\texttt{crosses\_p90} mean $2.69$), and dribbles (\texttt{dribbles\_p90} mean $3.68$).
  \item \textbf{Cluster 4 (Possession Controllers \& Hybrid Fullbacks -- 399 players):} Blends central/defensive midfielders ($48.6\%$, 194 players) and fullbacks/wingbacks ($45.9\%$, 183 players). This reflects modern tactical systems, where fullbacks and central midfielders share high passing volumes (\texttt{passes\_p90} mean $47.38$), pressing actions, and progressive ball circulation.
\end{itemize}

\subsection{Cluster Stability and Learnability}
To test whether the cluster partitions form learnable boundaries in feature space, a K-Nearest Neighbors (KNN) classifier was trained to predict cluster labels under 5-fold Stratified Cross-Validation across neighbor values $n\_neighbors \in [3, 15]$ (Table~\ref{tab:knn_sweep}).
...
\begin{minipage}{0.85\linewidth}
  ...
  \begin{tabularx}{\linewidth}{@{}>{\centering\arraybackslash}X >{\centering\arraybackslash}X >{\centering\arraybackslash}X@{}}
    \toprule
    \textbf{KNN Parameter (\texttt{n\_neighbors})} & \textbf{Mean CV Accuracy} & \textbf{Standard Deviation} \\
    \midrule
    \texttt{n\_neighbors} = 3 & 0.946 & $\pm 0.010$ \\
    \texttt{n\_neighbors} = 5 & 0.961 & $\pm 0.011$ \\
    \texttt{n\_neighbors} = 7 & 0.960 & $\pm 0.016$ \\
    \texttt{n\_neighbors} = 9 & 0.964 & $\pm 0.010$ \\
    \texttt{n\_neighbors} = 11 & 0.960 & $\pm 0.011$ \\
    \textbf{\texttt{n\_neighbors} = 15 (Final Model)} & \textbf{0.966} & \textbf{$\pm 0.008$} \\
    \bottomrule
  \end{tabularx}
\end{minipage}
\end{table}
...
Within this 152-player group, 12 features from the full 29-feature matrix are near-constant or uninformative (such as \texttt{clearances\_p90}, \texttt{tackles\_p90}, and \texttt{interceptions\_p90}) and are removed. In contrast, \texttt{passes\_p90} (retained from the macro pipeline) proves essential within the forward subspace to differentiate link-up creators from box-bound finishers. This 17-feature matrix is then scaled using \texttt{RobustScaler} fitted exclusively on the 152 strikers.
...
We test candidate values of $k \in [2, 7]$ on the scaled 17-feature striker data using the Elbow method and Silhouette Score (Figure~\ref{fig:striker_k_selection}), alongside the Gap Statistic analysis.
...
While exploratory screening of higher cluster counts (such as $k = 4$) might appear attractive to further differentiate attacking subgroups, closer inspection reveals a critical pathology in unsupervised sports analytics: \emph{clustering on player quality and raw output rather than tactical playing style}.
...
To avoid this quality-based distortion and isolate true tactical styles, we set $k = 3$. Under $k = 3$, forwards are partitioned according to their true spatial and behavioral role: elite volume scorers like Su\'arez, Ronaldo, Ag\"uero, and Benzema unite in the Poacher archetype, physical focal points populate the Target Man cluster, and versatile runners form the Deep-Lying Forward group.
...
\begin{minipage}{1.0\linewidth}
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{4.2cm}
    ...
    & ($n = 52$) & ($n = 82$) & ($n = 18$) \\
    \midrule
    \texttt{touches\_in\_box\_p90}  & 11.39        & 15.41        & \textbf{19.79} \\
    \texttt{shots\_p90}            & 2.17         & 2.44         & \textbf{3.75}  \\
    \texttt{np\_goals\_p90}        & 0.26         & 0.33         & \textbf{0.67}  \\
    \texttt{aerial\_won\_p90}      & 2.04         & \textbf{3.72}& 2.15           \\
    \texttt{passes\_p90}          & 28.40        & 22.40        & \textbf{30.89} \\
    \texttt{avg\_location\_x}     & 77.67        & 79.81        & \textbf{82.31} \\
    \texttt{pressures\_p90}       & \textbf{17.42}& 14.82       & 11.47          \\
    \bottomrule
  \end{tabularx}
\end{minipage}
\end{table}
...
\begin{itemize}
  \item \textbf{Poacher / Clinical Box Finisher (Cluster 2, $n = 18$):} Characterized by peak penalty-box presence ($\texttt{touches\_in\_box\_p90} = 19.79$), heavy shot volume ($3.75$ p90), high non-penalty goals ($0.67$ p90), and advanced pitch positioning ($\texttt{avg\_location\_x} = 82.31$), with lower defensive pressing ($11.47$ pressures p90). Examples include Luis Su\'arez, Cristiano Ronaldo, Gonzalo Higu\'ain, Sergio Ag\"uero, Daniel Sturridge, Karim Benzema, and Romelu Lukaku.
  \item \textbf{Target Man / Aerial Reference Point (Cluster 1, $n = 82$):} Physical forwards defined by aerial dominance ($\texttt{aerial\_won\_p90} = 3.72$, nearly double the other clusters) and hold-up play, along with the lowest passing volume in the forward cohort ($\texttt{passes\_p90} = 22.40$). Examples include Salom\'on Rond\'on, Mario Mand\v{z}uki\'c, Daniel Ciofani, Shane Long, Alberto Gilardino, and Borja Gonz\'alez Tom\'as (Borja Bast\'on).
  \item \textbf{Deep-Lying Forward (Cluster 0, $n = 52$):} Forwards who operate deeper into midfield ($\texttt{avg\_location\_x} = 77.67$), exhibiting substantial passing involvement ($\texttt{passes\_p90} = 28.40$) and the highest defensive pressing work rate ($\texttt{pressures\_p90} = 17.42$). Examples include Son Heung-Min, Theo Walcott, Nacer Chadli, Gerard Moreno, and Fabio Borini.
\end{itemize}
...
By looking at the cluster assignments of an opponent's starting lineup across recent matches, coaching staffs can construct an objective tactical overview of the team. For instance, knowing whether an opponent deploys two dynamic wide playmakers (Cluster 3) or inverted possession-controlling fullbacks/midfielders (Cluster 4) can help teams prepare their defensive structure and pressing tactics accordingly.
